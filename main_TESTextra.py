...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.X = None
        self.y = None
        self.main_button_1 = customtkinter.CTkButton(master=self, text= 'LOAD Train', fg_color="transparent", border_width=2, text_color=("gray10", "#DCE4EE"), command=self.open_file_dialog_train)
        self.main_button_1.grid(row=0, column=0, padx=(20, 20), pady=(20, 20), sticky="nsew")

        self.main_button_2 = customtkinter.CTkButton(master=self, text= 'LOAD Test', fg_color="transparent", border_width=2, text_color=("gray10", "#DCE4EE"), command=self.open_file_dialog_test)
        self.main_button_2.grid(row=0, column=1, padx=(20, 20), pady=(20, 20), sticky="nsew")

        self.quickload_button = customtkinter.CTkButton(master=self, text= 'QUICK LOAD', fg_color="transparent", border_width=2, text_color=("gray10", "#DCE4EE"), command=self.quick_load)
        self.quickload_button.grid(row=0, column=2, padx=(20, 20), pady=(20, 20), sticky="nsew")
        ...
        self.main_button_2.grid(row=0, column=3, padx=(20, 20), pady=(20, 20), sticky="nsew")
    ...
    # Function for choosing folder path and load TRAIN spectra
    def open_file_dialog_train(self):
        folderpath = customtkinter.filedialog.askdirectory()
        print(folderpath)
        # Read spectra
        self.X_train_df = readX_and_y(folderpath)

        if len(self.X_train_df) > 1:
            print('SUCCESS Loading TRAIN spectra')
            self.scale_button.configure(state='enabled')
        else:
            print('FAILED Loading TRAIN Spectra')

    # Function for choosing folder path and load TEST spectra
    def open_file_dialog_test(self):
        folderpath = customtkinter.filedialog.askdirectory()
        print(folderpath)
        # Read spectra
        self.X_test_df = readX_and_y(folderpath)

        if len(self.X_test_df) > 1:
            print('SUCCESS Loading TEST spectra')
            self.scale_button.configure(state='enabled')
        else:
            print('FAILED Loading TEST Spectra')

    def quick_load(self):
        folderpath_train = 'C:/Users/Bode/Desktop/PLS Pellets (gemahlen)/RELATIVE'
        print(folderpath_train)
        # Read spectra
        self.X_train_df = readX_and_y(folderpath_train)

        folderpath_test = 'C:/Users/Bode/Desktop/PLS Pellets (gemahlen)/RELATIVE_SWIR'
        print(folderpath_test)
        # Read spectra
        self.X_test_df = readX_and_y(folderpath_test)

        if len(self.X_train_df) > 1 and len(self.X_test_df) > 1:
    ...
    def do_scaling(self):
        ...
        # Scale Train Spectra
        self.X_train_df_specific = self.X_train_df.loc[:, self.preset_startWL:self.preset_stopWL]
        self.X_train = self.X_train_df_specific.to_numpy()
        self.y_train = np.array(self.X_train_df_specific.index)
        self.n_wavelenths = self.X_train.shape[1]
        ...
        plot_samples(self.wl_int, self.X_train, self.y_train)
        # Scale Test Spectra
        self.X_test_df_specific = self.X_test_df.loc[:, self.preset_startWL:self.preset_stopWL]
        self.X_test = self.X_test_df_specific.to_numpy()
        self.y_test = np.array(self.X_test_df_specific.index)
    ...
    def do_preprocessing(self):
        ...
        if self.check_savgol.get() == True and self.check_snv.get() == False:
            self.X_train = savgol(self.X_train, savgol_wl, savgol_poly, deriv=savgol_deriv)
            self.X_test = savgol(self.X_test, savgol_wl, savgol_poly, deriv=savgol_deriv)
            print('Savgol done')
        
        elif self.check_savgol.get() == False and self.check_snv.get() == True:
            self.X_train = snv(self.X_train)
            self.X_test = snv(self.X_test)
            print('SNV done')
        ...
        plt.plot(self.wl_int, self.X_train[:20,:].T)
    ...
    def reset(self):
        ...
        self.X_train_df_specific = None
        self.X_test_df = None
        self.X_test_df_specific = None
